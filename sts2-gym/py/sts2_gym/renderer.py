...
def render_text(obs_payload: dict[str, Any], mask_payload: dict[str, Any] | None = None) -> str:
    """Top-level renderer — dispatches on ``phase``.

    Day-7 P0: only combat is rendered in detail. Other phases get a single-line
    summary until ICardSelector hooks land (dev plan §2.3).
    """
    ...
    if phase == "combat":
        body = render_combat(obs_payload, mask_payload)
    else:
        body = f"(no detailed renderer for phase={phase!r} yet — Day-7 covers combat only)"

    return f"=== STS2 obs ({header}) ===\n{body}"


__all__ = ["render_text", "render_combat", "strip_bbcode"]
