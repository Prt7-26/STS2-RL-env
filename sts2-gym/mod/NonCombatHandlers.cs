...
internal static class NonCombatHandlers
{
    ...
    public static async Task<(int, string)> ChooseMapNodeAsync(JsonElement cmd)
    {
        ...
        // Day-14.9: removed the post-EnterMapCoord FastDelay.Of(200). With
        // NTransition.RoomFadeIn patched (Day-14.8), EnterMapCoord already
        // awaits the full room transition; the action queue is empty at this
        // point (drain=0 confirmed by Day-14.7 instrumentation). The 200ms
        // paranoid wait was burning 33-164ms per map step for nothing.
